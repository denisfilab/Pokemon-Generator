...
function generateVisualDescriptions(
  ...
  for (let i = 0; i < evolvement + 1; i++) {
    let accessoriesFeatures = `${
      ...
      accessories[1] ? `this pokémon has detail of a ${accessories[1]},` : ''
    } ${accessories[2] ? `this pokémon is wearing a ${accessories[2]}, ` : ''}`
...
async function generateAbilityAndName(description: string, element: Element) {
  ...
  try {
    ...
    console.error('An error occurred while generating ability and name:', error)
...
async function generateCard(
  ...
  for (let i = 0; i <= evolvement; i++) {
    try {
      ...
      let ability = await generateAbility(tempVisualDescription, elements, i)
...
async function generateImagesForCards(cards: {
  [key: string]: Card
}): Promise<{ [key: string]: Card }> {
  const imageGenerationPromises = Object.entries(cards).map(
    async ([key, card]) => {
      try {
        ...
        const imageFileName = `${card.index}_${card.name.toLowerCase()}.png`
        ...
        console.error(
          `Error generating image for card ${card.name}_${card.index}.png:`,
  ...
  results.forEach((result) => {
    if ('error' in result) return // Skip updating if there was an error

    const card = cards[result.key]
    if (card) {
